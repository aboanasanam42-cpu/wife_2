...
class MultiSlotBot:
    ...
    def save_state(self):
        data = {"version": 2, "max_slots": self.max_slots, "realized_pnl_usdt": self.realized_pnl, "trades": self.trades, "slots": self.slots, "updated_at": datetime.now(timezone.utc).isoformat()}
    ...
    def run_once(self):
        ...
        with ThreadPoolExecutor(max_workers=max(1, len(self.cfg.trade_symbols))) as pool:
            futures = {pool.submit(self.process_symbol, symbol): symbol for symbol in self.cfg.trade_symbols}
    ...
    def run(self):
        ...
        self.notifier.notify_startup({"pairs": ", ".join(self.cfg.trade_symbols), "slot": self.cfg.slot_size_usdt, "max_slots": self.max_slots, "reserve": self.cfg.cash_reserve_usdt, "timeframe": self.cfg.timeframe, "interval": self.cfg.poll_interval_seconds, "simulation": self.cfg.simulation_mode})
        delay = 5
        while self.running:
